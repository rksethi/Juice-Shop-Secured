...
export function login () {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    // SECURITY FIX: Check account lockout before attempting login
    const lockoutKey = `login_attempts_${req.body.email || req.ip}`
    const lockoutAttempts = (global as any).loginAttempts?.[lockoutKey] || 0
    const lockoutTime = (global as any).loginLockoutTime?.[lockoutKey] || 0
    const maxAttempts = 5
    ...
    if (lockoutTime > Date.now()) {
      ...
      res.status(429).json({ 
        error: `Account temporarily locked due to too many failed login attempts. Please try again in ${remainingMinutes} minute(s).` 
      })
      return
    }
    
    // SECURITY FIX: Use parameterized query to prevent SQL injection, fetch user first then verify password
    try {
      const [users]: any = await models.sequelize.query(
      ...
      // SECURITY FIX: Verify password using secure method (supports both bcrypt and MD5 for migration)
      const passwordMatch = await security.verifyPassword(req.body.password || '', user.data.password)
      ...
      if ((global as any).loginAttempts) {
        delete (global as any).loginAttempts[lockoutKey]
        delete (global as any).loginLockoutTime[lockoutKey]
      }
  ...
  // SECURITY FIX: Account lockout helper function
  function incrementFailedAttempts (key: string) {
    if (!(global as any).loginAttempts) {
      (global as any).loginAttempts = {}
      (global as any).loginLockoutTime = {}
    }
    const attempts = ((global as any).loginAttempts[key] || 0) + 1
    ;(global as any).loginAttempts[key] = attempts
    
    if (attempts >= 5) {
      ;(global as any).loginLockoutTime[key] = Date.now() + (15 * 60 * 1000) // Lock for 15 minutes
    }
  }
