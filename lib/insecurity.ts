...
// SECURITY FIX: Load private key from environment variable instead of hardcoding
// Fallback to hardcoded key only if env var not set (for backward compatibility during migration)
const privateKey = process.env.JWT_PRIVATE_KEY || (fs && fs.existsSync('encryptionkeys/jwt.key') ? fs.readFileSync('encryptionkeys/jwt.key', 'utf8') : '-----BEGIN RSA PRIVATE KEY-----\r\\r\n-----END RSA PRIVATE KEY-----')
...
// Keep MD5 hash function for backward compatibility with existing data, but use bcrypt for new passwords
export const hash = (data: string) => crypto.createHash('md5').update(data).digest('hex')
// New secure password hashing function using bcrypt
...
export const verifyPassword = async (password: string, hash: string): Promise<boolean> => {
  ...
  // Fallback to MD5 for backward compatibility with existing passwords
